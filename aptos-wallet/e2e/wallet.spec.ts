...
test('wallet menu refreshes only that wallet and highlights its changed accounts', async ({ page }, testInfo) => {
  ...
  await page.goto('/')
  await page.getByRole('button', { name: /日常钱包/ }).click()
  const actionMenu = page.getByLabel('日常钱包 更多操作')
  await actionMenu.click()
  await expect(page.getByRole('button', { name: '刷新钱包余额' })).toBeVisible()
  await page.getByRole('heading', { name: '我的钱包' }).click()
  await expect(page.getByRole('button', { name: '刷新钱包余额' })).toBeHidden()
  await actionMenu.click()
