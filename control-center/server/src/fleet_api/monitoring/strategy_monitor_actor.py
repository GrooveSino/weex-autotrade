...
def merge_actor_waits(
    ...
    if queue_wait is None:
        return waits
    superseded = {queue_wait.key}
    if actor is not None and actor.queue_phase == "close":
        superseded.add("hold")
    elif actor is not None and actor.queue_phase == "open":
        superseded.add("round-gap")
    return [wait for wait in waits if wait.key not in superseded and not wait.key.startswith("cycle-stage:")] + [
        queue_wait
    ]
