...
class FleetWriteCoordinator:
    ...
    def discard_low_priority(self, key: str) -> bool:
        """Drop one pending heartbeat that a newer durable event supersedes.

        The callback has not started while it remains in ``_low``.  Completing
        its Future with ``None`` preserves the coalescing contract for callers
        that attach a best-effort publication callback, without consuming the
        writer ahead of a critical order or lifecycle boundary.
        """
        with self._lock:
            write = self._low.pop(key, None)
        if write is None or write.future.done():
            return False
        write.future.set_result(None)
        return True
