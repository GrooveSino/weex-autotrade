...
from fleet_api.campaign_actor_phases import CampaignActorPhases
...
def test_hold_timer_starts_only_after_both_open_positions_are_verified(monkeypatch) -> None:
    ...
    btc, eth = _plans()
    monkeypatch.setattr(
        "fleet_api.campaign_actor_phases._size_cycle",
        lambda _plan, _lanes, _quote: (btc, eth, {"opening_notional_quote": "100"}),
    )
    monkeypatch.setattr(
        "fleet_api.campaign_actor_phases.observe_positions",
...
def test_close_stage_rebuilds_lanes_from_the_persisted_child_plan(monkeypatch) -> None:
    ...
    monkeypatch.setattr("fleet_api.campaign_actor_phases.close_lanes", lambda *_args: [])
    monkeypatch.setattr(
        "fleet_api.campaign_actor_phases.observe_positions",
        lambda *_args, **_kwargs: {"BTC": Decimal("0"), "ETH": Decimal("0")},
    )
    monkeypatch.setattr("fleet_api.campaign_actor_phases.positions_are_flat", lambda *_args: True)
...
def test_stop_uses_emergency_safe_stop_instead_of_a_normal_close_phase(monkeypatch) -> None:
    ...
    called: list[str] = []
    monkeypatch.setattr(
        "fleet_api.campaign_actor_phases.safe_stop",
