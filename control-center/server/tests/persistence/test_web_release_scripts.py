...
def test_web_release_publish_and_rollback_only_switches_static_symlink(tmp_path: Path) -> None:
    control_center = Path(__file__).parents[3]
    deploy = control_center / "scripts" / "macos" / "release" / "deploy-web-release.zsh"
    rollback = control_center / "scripts" / "macos" / "release" / "rollback-web-release.zsh"
...
def test_service_release_and_launch_agent_scripts_use_stable_paths_without_credentials() -> None:
    control_center = Path(__file__).parents[3]
    deploy = (control_center / "scripts" / "macos" / "release" / "deploy-service-release.zsh").read_text(
        encoding="utf-8"
    )
    installer = (control_center / "scripts" / "macos" / "runtime" / "install-launch-agents.zsh").read_text(
        encoding="utf-8"
    )
    activate = (control_center / "scripts" / "macos" / "release" / "activate-service-release.zsh").read_text(
        encoding="utf-8"
    )
...
def test_service_activation_refuses_active_legacy_worker_before_launching_executor(tmp_path: Path) -> None:
    control_center = Path(__file__).parents[3]
    activate = control_center / "scripts" / "macos" / "release" / "activate-service-release.zsh"
    release_root = tmp_path / "runtime"
    home = tmp_path / "home"
