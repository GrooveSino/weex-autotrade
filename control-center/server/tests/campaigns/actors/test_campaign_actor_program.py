...
def test_actor_emits_canonical_timer_completion_events_before_advancing() -> None:
    class GapPhases(_Phases):
        def __init__(self) -> None:
            super().__init__(hold_seconds=0.02)
            self.close_calls = 0
        ...
        def close(self, _campaign: object, opened: OpenCycle) -> CloseCycle:
            ...
            if self.close_calls == 1:
                ...
                return CloseCycle(Decimal(0), None, None, None, 0.02)
            return CloseCycle(Decimal(0), {"status": "completed", "reason": "done"}, None, None, 0)
    ...
    assert capacity.admit("one")
    future = runtime.start("one", "account-one", _program(phases, result, events=events))
    try:
        future.result(timeout=3)
