...
import { countdown, displayTime, levelLabel, quote } from './logDrawerFormat'
...
interface Props {
  ...
  primaryWait: StrategyMonitorSnapshot['activeWaits'][number] | null
  ...
  return <div className="drawer-backdrop" role="presentation" onMouseDown={onClose}>
    <aside className="log-drawer execution-monitor" role="dialog" aria-modal="true" aria-labelledby="log-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <section className={`runtime-stage-strip ${primaryWait ? 'waiting' : ''}`} aria-label="当前运行阶段">
        <div className="runtime-stage-main">{primaryWait ? <LoaderCircle className="spin" size={15} /> : <CircleDot size={13} />}<span><small>{primaryWait?.label ?? '当前阶段'}</small><strong>{monitor?.phase ?? '等待执行状态'}</strong></span></div>
        ...
        <div className="runtime-stage-countdown"><small>{primaryWait ? (primaryWait.key === 'hold' ? '持仓剩余' : primaryWait.key === 'round-gap' ? '下轮开始' : '等待剩余') : '倒计时'}</small><strong>{countdown(primaryWaitRemaining)}</strong></div>
      </section>
